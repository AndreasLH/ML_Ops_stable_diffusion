...
def visualize(datapath, n=6, m=3):
    assert isinstance(n, int) is True, "n must be an integer"
    assert isinstance(m, int) is True, "m must be an integer"
    ...
    for i, image in enumerate(train_dataset[: m * n]["images"]):
        ...
        axs[j, i].imshow(
            image.permute(1, 2, 0)
        )  # permute to H x W x Channel, because that is what matplotlib wants
